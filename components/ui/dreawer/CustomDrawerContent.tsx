...
import { Home, LogOut } from "@tamagui/lucide-icons";
...
export default function CustomDrawerContent(props: any) {
    ...
    <DrawerContentScrollView {...props}>
      <XStack $sm={{ flexDirection: "column"  }}>
        <YGroup width={300} size="$4">
          {props.state.routeNames.map((name: any, index: number) =>
            CutomDrawerContentItem(index, name)
          )}

        <YGroup width={300} size="$4">
          <YGroup.Item >
            <ListItem hoverTheme icon={LogOut} title={'Salir'} onPress={() => cerrarSession()} />
          </YGroup.Item>
        </YGroup>
  ...
  function CutomDrawerContentItem(index: number, name: any) {
    return (
      <YGroup.Item key={index.toString()}>
        <ListItem hoverTheme icon={Home} title={name} />
      </YGroup.Item>
    );
  }
}
